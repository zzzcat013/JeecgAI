...
<template>
  <div class="p-2">
            ...
            <a-row :span="24" class="knowledge-row">
              <a-col :xxl="4" :xl="6" :lg="6" :md="6" :sm="12" :xs="24" v-if="!type || type==='knowledge'">
                <a-card class="add-knowledge-card" :bodyStyle="cardBodyStyle">
                  ...
                  <div class="add-knowledge-doc" @click="() => handleLocalImport()">
                    <Icon icon="ant-design:folder-open-outlined" size="13"></Icon><span>本地打包导入</span>
                  </div>
                  ...
                  <div class="flex" style="justify-content: space-between">
                    <div class="card-text">
                      ...
                      <div v-else-if="item.status==='building'" class="card-text-status">
                        <a-tooltip :title="getDocStatusText(item)">
                          <div class="status-building">
                            <Icon icon="ant-design:loading-outlined" spin size="14"></Icon>
                      ...
                      <div v-else-if="item.status==='wait_confirm'" class="card-text-status">
                        <Icon icon="ant-design:picture-outlined" size="16" color="#1890ff"></Icon>
                        <span class="ml-2">待确认图片</span>
                      </div>
                      <a-tooltip v-else-if="item.status==='failed'" :title="getDocStatusText(item)">
                    ...
                    <a-dropdown placement="bottomRight" :trigger="['click']">
                      ...
                      <template #overlay>
                        <a-menu>
                          ...
                          <a-menu-item key="viewContent" @click="handleViewContent(item)">
    ...
    <!--  本地打包导入  -->
    <AiragLocalImportModal @register="localImportRegister" @success="handleSuccess"></AiragLocalImportModal>
    <!--  文本明细  -->
...
<script lang="tsx">
  ...
  export default {
    ...
    setup(props, { emit }) {
      ...
      const [localImportRegister, { openModal: localImportOpenModal }] = useModal();
       ...
       * 本地打包导入
       */
      function handleLocalImport(localPath?: string, originDocId?: string) {
        localImportOpenModal(true, { knowledgeId: knowledgeId.value, localPath, originDocId });
      }
      ...
      function getDocStatusText(item) {
        if (item.status === 'building') {
          try {
            const metadata = JSON.parse(item.metadata || '{}');
            return metadata.buildStep || '构建中...';
          } catch (e) {
            return '构建中...';
          }
        }
        if (item.status === 'complete') return '解析完成';
        if (item.status === 'failed') return '构建失败';
        if (item.status === 'wait_confirm') return '待确认图片';
        return '草稿';
      }
      ...
      // 已提示过包含图片的文档ID
      const promptedDocIds = new Set<string>();
          ...
          if (res.success) {
            ...
            if(res.result.records){
              ...
              for (const item of res.result.records) {
                // 查找旧数据中的 loading 状态
                const oldItem = knowledgeDocDataList.value.find(i => i.id === item.id);
                const wasBuilding = oldItem && oldItem.status === 'building';
                ...
                if(item.status && item.status === 'building' ){
                  ...
                  item.loading = false;
                  // 如果是等待确认状态，或者状态从 building 变为 complete，触发提示
                  if (item.status === 'wait_confirm' || (wasBuilding && item.status === 'complete')) {
                    checkAndNotifyImages(item);
                  }
                }
              }
              // 第一次加载完成后，标记为非首次加载
       ...
       * 检查文档是否包含图片并弹出提示
       */
      function checkAndNotifyImages(item) {
        if (!promptedDocIds.has(item.id)) {
          try {
            const metadata = JSON.parse(item.metadata || '{}');
            if (metadata.hasImages) {
              promptedDocIds.add(item.id);
              
              const modal = Modal.confirm({
                title: '解析发现图片资源',
                width: 600,
                icon: h(Icon, { icon: 'ant-design:picture-outlined', color: '#1890ff', size: 24 }),
                content: () => (
                  <div style="margin-top: 10px">
                    <p style="font-size: 14px">文档 <b>{item.title}</b> 解析完成，但检测到其中包含 <b>图片资源</b>。</p>
                    <p style="color: #666">为了获得最佳的图文混排效果，建议使用“本地打包导入”模式。您也可以选择忽略图片，仅导入纯文本内容。</p>
                    <div style="margin-top: 15px">
                      <a href="https://help.jeecg.com/aigc/guide/knowledge#4-%E7%9F%A5%E8%AF%86%E5%BA%93%E6%96%87%E6%A1%A3" target="_blank">
                        <Icon icon="ant-design:link-outlined" size="14" /> 查看文档库上传规范
                      </a>
                    </div>
                  </div>
                ),
                ...
                cancelText: '仅导入纯文本',
                onOk: () => {
                  handleLocalImport(metadata.mineruOutputPath, item.id);
                },
                onCancel: async () => {
                  // 用户选择仅导入纯文本
                  try {
                    await knowledgeConfirmSingle({ docId: item.id });
                    createMessage.success('已开始导入纯文本内容');
                    reload();
                  } catch (e) {
                    console.error('确认单文档导入失败', e);
                  } finally {
                    modal.destroy();
                  }
                }
       ...
       * 查看全文
       */
      function handleViewContent(record) {
        docTextDescOpenModal(true, { ...record });
      }
